 ...
 *
 */
/datum/piping_minigame

	var/finished = FALSE

	var/board_size = 8

	var/list/datum/piping_minigame_piece/board

/datum/piping_minigame/New(board_size)
	. = ..()
 ...
 * returns finished.
 */
/datum/piping_minigame/proc/game_check()
 ...
 */
/datum/piping_minigame/proc/generate()
	board = list()
	...
	for(var/i in 1 to board_size)
		for(var/k in 1 to board_size)
			board[i][k] = new /datum/piping_minigame_piece(src)
	...
	while(possible_moves.len)
		...
		board[current_x][current_y].pass_in = GLOB.flip_dir[move]
 ...
 */
/datum/piping_minigame/proc/get_simplified_image()
...
/datum/piping_minigame/proc/consider_possible_moves(xcord as num,ycord as num)
...
/datum/piping_minigame/proc/check_connections()
	...
	while(queued_coord_list.len > counter)
		...
		if(counter > board_size*board_size)
			log_runtime("Counter runoff stopped occured!")
			break
...
/datum/piping_minigame_piece
	...
	var/datum/piping_minigame/game

/datum/piping_minigame_piece/New(datum/piping_minigame_piece/_game)
	. = ..()
	game = _game

///Returns value of pass_in + pass_out
/datum/piping_minigame_piece/proc/get_dir_val()
	if(pass_in != NONE && pass_out != NONE)
		return pass_in + pass_out
	return 0

///Returns a list of possible directions this cell can connect to
/datum/piping_minigame_piece/proc/get_dir_val_list()
...
/datum/piping_minigame_piece/proc/rotate(num = 1)
