...
/obj/item/storage/belt/rogue/pouch/gempouch/Initialize()
	. = ..()
	var/obj/item/roguegem/random/no_riddle/randomgem = new(loc)
	if(istype(randomgem))
		if(!SEND_SIGNAL(src, COMSIG_TRY_STORAGE_INSERT, randomgem, null, TRUE, TRUE))
			qdel(randomgem)
	var/obj/item/roguegem/random/no_riddle/randomgem2 = new(loc)
	if(istype(randomgem2))
		if(!SEND_SIGNAL(src, COMSIG_TRY_STORAGE_INSERT, randomgem2, null, TRUE, TRUE))
			qdel(randomgem2)
	var/obj/item/roguegem/yellow/toper = new(loc)
	if(istype(toper))
		if(!SEND_SIGNAL(src, COMSIG_TRY_STORAGE_INSERT, toper, null, TRUE, TRUE))
			qdel(toper)
	var/obj/item/roguegem/yellow/toper2 = new(loc)
	if(istype(toper2))
		if(!SEND_SIGNAL(src, COMSIG_TRY_STORAGE_INSERT, toper2, null, TRUE, TRUE))
			qdel(toper2)
